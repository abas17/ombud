...
where id_reg = ?";
    ...
    if (isset($row['id_reg'])) {
        if (isset($_POST['button_update'])) {
            $status = 'Pemeriksaan';
            if ($_POST['laporan_diterima'] == 'Terima') {
                $durasi = 'Proses';
            } else {
                $durasi = 'Berhenti';
            }
            $petugas_riksa = $_SESSION['user']['nama'];
            $updateSql = "UPDATE registrasi_lap_masyarakat SET laporan_diterima = ?, tanggal_diterima = ?, disposisi_ke = ?, status = ?, durasi = ?, petugas_riksa = ? WHERE id_reg = ?";
            $stmt = $db->prepare($updateSql);
            $stmt->bindParam(1, $_POST['laporan_diterima']);
            $stmt->bindParam(2, $_POST['tanggal_diterima']);
            $stmt->bindParam(3, $_POST['disposisi_ke']);
            $stmt->bindParam(4, $status);
            $stmt->bindParam(5, $durasi);
            $stmt->bindParam(6, $petugas_riksa);
            $stmt->bindParam(7, $id);
            if ($stmt->execute()) {
                $_SESSION['hasil'] = true;
            ...
            echo '<meta http-equiv="refresh" content="0; url=?page=pemeriksaan">';
        }
                    ...
                    <div class="row">
                        ...
                        <div class="col s12 m6 l6 right-align-md">
                            <ol class="breadcrumbs mb-0">
                                ...
                                <li class="breadcrumb-item"><a href="?page=pemeriksaan">Pemeriksaan</a>
                                </li>
                            </ol>
                        </div>
                    </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col s12">
                                        <div class="input-field col s12">
                                            ...
                                            <p>
                                                <label>
                                                    <input name="laporan_diterima" type="radio" <?php if ($row['laporan_diterima'] == 'Terima') echo 'checked'; ?> value="Terima" />
                                                    <span>Terima</span>
                                                </label>
                                            </p>
                                            <p>
                                                <label>
                                                    <input name="laporan_diterima" type="radio" <?php if ($row['laporan_diterima'] == 'Tolak') echo 'checked'; ?> value="Tolak" />
                                                    <span>Tolak</span>
                                                </label>
                                            </p>
                                        </div>
                                        <div class="input-field col s12">
                                            ...
                                            <input required value="<?php echo $row['tanggal_diterima'] ?>" type="date" id="tanggal_diterima" name="tanggal_diterima">
                                        </div>
                                        <div class="input-field col s12">
                                            <div>
                                                <label for="disposisi_ke">Laporan Didisposisikan kepada</label>
                                            </div>
                                            <select class="select2 browser-default" name="disposisi_ke" id="disposisi_ke">
                                                <option value="" disabled selected>---Didisposisikan kepada---</option>
                                                <?php
                                                ...
                                                while ($rowd = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                                    if ($rowd["nama"] == $disposisi_ke) {
                                                        echo "<option value='" . $rowd["nama"] . "' selected>" . $rowd["nama"] . "</option>";
                                                    } else {
