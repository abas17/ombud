...
where id_reg = ?";
    ...
    if (isset($row['id_reg'])) {
        if (isset($_POST['button_update'])) {
            $status = 'Verifikasi';
            if ($_POST['verifikasi'] == 'Memenuhi Syarat') {
                $durasi = 'Proses';
            } else {
                $durasi = 'Berhenti';
            }
            $petugas_verifikasi = $_SESSION['user']['nama'];
            $updateSql = "UPDATE registrasi_lap_masyarakat SET verifikasi = ?, status = ?, durasi = ?, petugas_verifikasi = ? WHERE id_reg = ?";
            $stmt = $db->prepare($updateSql);
            $stmt->bindParam(1, $_POST['verifikasi']);
            $stmt->bindParam(2, $status);
            $stmt->bindParam(3, $durasi);
            $stmt->bindParam(4, $petugas_verifikasi);
            $stmt->bindParam(5, $id);
            if ($stmt->execute()) {
                $_SESSION['hasil'] = true;
            ...
            echo '<meta http-equiv="refresh" content="0; url=?page=verifikasi">';
        }
                                    ...
                                    <div class="col s12">
                                        <div class="col s12 input-field">
                                            <select name="verifikasi">
                                                ...
                                                <option value="Belum Lengkap" <?php if ($row['verifikasi'] == 'Belum Lengkap') echo 'selected'; ?>>Belum Lengkap</option>
                                                <option value="Memenuhi Syarat" <?php if ($row['verifikasi'] == 'Memenuhi Syarat') echo 'selected'; ?>>Memenuhi Syarat</option>
                                                <option value="Berhenti" <?php if ($row['verifikasi'] == 'Berhenti') echo 'selected'; ?>>Berhenti</option>
                                                <option value="Tidak Memenuhi Syarat dan Tidak Lengkap" <?php if ($row['verifikasi'] == 'Tidak Memenuhi Syarat dan Tidak Lengkap') echo 'selected'; ?>>Tidak Memenuhi Syarat dan Tidak Lengkap</option>
                                            </select>
